(() => {
  ...
  var stringifySymbol = (v3, i = "") => {
    ...
    return (
      ...
      isSymbol(v3) ? `Symbol(${(_a2 = v3.description) != null ? _a2 : i})` : v3
    );
  };
  ...
  function watch(source, cb, options = EMPTY_OBJ) {
    ...
    const job = (immediateFirstRun) => {
      ...
      if (cb) {
        ...
        if (deep || forceTrigger || (isMultiSource ? newValue.some((v3, i) => hasChanged(v3, oldValue[i])) : hasChanged(newValue, oldValue))) {
          if (cleanup) {
            cleanup();
          }
    ...
    if (fallthroughAttrs && inheritAttrs !== false) {
      ...
      if (keys.length) {
        if (shapeFlag & (1 | 6)) {
          ...
          const eventAttrs = [];
          const extraAttrs = [];
          for (let i = 0, l3 = allAttrs.length; i < l3; i++) {
...
For more details, see https://link.vuejs.org/feature-flags.`
  ...
  function baseCreateRenderer(options, createHydrationFns) {
    ...
    const setupRenderEffect = (instance, initialVNode, container, anchor, parentSuspense, namespace, optimized) => {
      const componentUpdateFn = () => {
        if (!instance.isMounted) {
          ...
          const { bm, m: m3, parent, root, type } = instance;
    ...
    const unmountComponent = (instance, parentSuspense, doRemove) => {
      ...
      const { bum, scope, job, subTree, um, m: m3, a } = instance;
      invalidateMount(m3);
      invalidateMount(a);
      if (bum) {
        invokeArrayFns(bum);
...
Expected function or array of functions, received type ${typeof value}.`
  ...
  function setSelected(el2, value) {
    ...
    for (let i = 0, l3 = el2.options.length; i < l3; i++) {
      ...
      if (isMultiple) {
        if (isArrayValue) {
          ...
          if (optionType === "string" || optionType === "number") {
            option.selected = value.some((v3) => String(v3) === String(optionValue));
          } else {
...
- For vite: pass it via @vitejs/plugin-vue options. See https://github.com/vitejs/vite-plugin-vue/tree/main/packages/plugin-vue#example-for-passing-options-to-vuecompiler-sfc`;
  ...
  function useIntervalFn(cb, interval = 1e3, options = {}) {
    ...
    if (isRef2(interval) || typeof interval === "function") tryOnScopeDispose(watch2(interval, () => {
...
`), r2 = "", i = "", s2 = [];
        for (; n2.length > 0; ) {
          let a = false, o2 = [], p3;
          for (p3 = 0; p3 < n2.length; p3++) if (this.rules.other.blockquoteStart.test(n2[p3])) o2.push(n2[p3]), a = true;
          else if (!a) o2.push(n2[p3]);
          else break;
          n2 = n2.slice(p3);
          let u2 = o2.join(`
...
`, e2 = e2.substring(Z.length + 1), g = A.slice(f2);
        ...
        i.raw = i.raw.trimEnd();
        for (let p3 = 0; p3 < i.items.length; p3++) if (this.lexer.state.top = false, i.items[p3].tokens = this.lexer.blockTokens(i.items[p3].text, []), !i.loose) {
          let u2 = i.items[p3].tokens.filter((g) => g.type === "space"), c2 = u2.length > 0 && u2.some((g) => this.rules.other.anyLine.test(g.raw));
          i.loose = c2;
        }
        if (i.loose) for (let p3 = 0; p3 < i.items.length; p3++) i.items[p3].loose = true;
        return i;
      }
    }
    html(e2) {
...
`) : [], s2 = { type: "table", raw: t2[0], header: [], align: [], rows: [] };
      if (n2.length === r2.length) {
        ...
        for (let a of i) s2.rows.push(V(a, s2.header.length).map((o2, p3) => ({ text: o2, tokens: this.lexer.inline(o2), header: false, align: s2.align[p3] })));
        return s2;
      }
    }
    lheading(e2) {
...
` ? t2[1].slice(0, -1) : t2[1];
    ...
    emStrong(e2, t2, n2 = "") {
      ...
      if (!(r2[1] || r2[2] || "") || !n2 || this.rules.inline.punctuation.exec(n2)) {
        let s2 = [...r2[0]].length - 1, a, o2, p3 = s2, u2 = 0, c2 = r2[0][0] === "*" ? this.rules.inline.emStrongRDelimAst : this.rules.inline.emStrongRDelimUnd;
        for (c2.lastIndex = 0, t2 = t2.slice(-1 * e2.length + s2); (r2 = c2.exec(t2)) != null; ) {
          ...
          if (p3 -= o2, p3 > 0) continue;
          o2 = Math.min(o2, o2 + p3 + u2);
...
` + r2.raw, this.inlineQueue.at(-1).src = s2.text) : this.tokens.links[r2.tag] || (this.tokens.links[r2.tag] = { href: r2.href, title: r2.title }, t2.push(r2));
        ...
        if (this.options.extensions?.startBlock) {
          ...
          this.options.extensions.startBlock.forEach((p3) => {
            o2 = p3.call({ lexer: this }, a), typeof o2 == "number" && o2 >= 0 && (s2 = Math.min(s2, o2));
...
` + r2.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = s2.text) : t2.push(r2);
    ...
    inlineTokens(e2, t2 = []) {
      ...
      for (; e2; ) {
        ...
        if (o2 = this.tokenizer.inlineText(p3)) {
      ...
      return e2.forEach((n2) => {
        ...
        if (r2.async = this.defaults.async || r2.async || false, n2.extensions && (n2.extensions.forEach((i) => {
          ...
          for (let s2 in n2.renderer) {
            ...
            let a = s2, o2 = n2.renderer[a], p3 = i[a];
            i[a] = (...u2) => {
              let c2 = o2.apply(i, u2);
              return c2 === false && (c2 = p3.apply(i, u2)), c2 || "";
            };
          }
          r2.renderer = i;
        }
        if (n2.tokenizer) {
          ...
          for (let s2 in n2.tokenizer) {
            ...
            let a = s2, o2 = n2.tokenizer[a], p3 = i[a];
            i[a] = (...u2) => {
              let c2 = o2.apply(i, u2);
              return c2 === false && (c2 = p3.apply(i, u2)), c2;
            };
          ...
          for (let s2 in n2.hooks) {
            ...
            S.passThroughHooks.has(s2) ? i[a] = (u2) => {
              ...
              if (this.defaults.async) return (async () => {
                ...
                return g === false && (g = await p3.apply(i, u2)), g;
              })();
              let c2 = o2.apply(i, u2);
              return c2 === false && (c2 = p3.apply(i, u2)), c2;
            };
    ...
    parseMarkdown(e2) {
      return (n2, r2) => {
        ...
        try {
          ...
          let p3 = (s2.hooks ? s2.hooks.provideLexer() : e2 ? x.lex : x.lexInline)(n2, s2);
          s2.hooks && (p3 = s2.hooks.processAllTokens(p3)), s2.walkTokens && this.walkTokens(p3, s2.walkTokens);
          let c2 = (s2.hooks ? s2.hooks.provideParser() : e2 ? b.parse : b.parseInline)(p3, s2);
...
Please report this to https://github.com/markedjs/marked.`, e2) {
  ...
  function markedHighlight(options) {
    ...
    return {
      ...
      walkTokens(token) {
        ...
        if (options.async) {
          return Promise.resolve(options.highlight(token.text, lang2, token.lang || "")).then(updateToken(token));
        }
        const code = options.highlight(token.text, lang2, token.lang || "");
      ...
      renderer: {
        code(code, infoString, escaped) {
          ...
          const classValue = lang2 ? options.langPrefix + escape(lang2) : options.emptyLangClass;
  ...
  var ia = (e2) => {
    ...
    return e2.replace(na, (t2, a, n2) => {
      ...
      const p3 = `<span style="color: #${o2}">${a}</span>`;
      return l3 = ++l3 % qe2.length, p3;
    });
  };
 ...
 Current word number: $2`, "Anonymous", "Dwarves", "Hobbits", "Ents", "Wizards", "Elves", "Maiar", "GIF", "Search GIF", "Profile", "Approved", "Waiting", "Spam", "Unsticky", "Oldest", "Latest", "Hottest", "What do you think?"]);
  var Ge2 = X2(["Nombre de usuario", "El nombre de usuario no puede tener menos de 3 bytes.", "Correo electr\xF3nico", "Por favor confirma tu direcci\xF3n de correo electr\xF3nico.", "Sitio web", "Opcional", "Comenta aqu\xED...", "Sin comentarios todav\xEDa.", "Enviar", "Like", "Anular like", "Responder", "Anular respuesta", "Comentarios", "Recargar", "Cargar M\xE1s...", "Previsualizar", "Emoji", "Subir Imagen", "segundos atr\xE1s", "minutos atr\xE1s", "horas atr\xE1s", "d\xEDas atr\xE1s", "justo ahora", "Subiendo", "Iniciar sesi\xF3n", "cerrar sesi\xF3n", "Admin", "Fijado", "Palabras", `Por favor escriba entre $0 y $1 palabras!
 ...
 Nombre actuel de mots : $2`, "Anonyme", "Nains", "Hobbits", "Ents", "Mages", "Elfes", "Ma\xEFar", "GIF", "Rechercher un GIF", "Profil", "Approuv\xE9", "En attente", "Ind\xE9sirable", "D\xE9tacher", "Le plus ancien", "Dernier", "Le plus populaire", "Qu'en pensez-vous ?"]);
  var Ze2 = X2(["Nickname", "Il nickname non pu\xF2 avere meno di 3 caratteri.", "E-mail", "Per favore conferma il tuo indirizzo e-mail.", "Sito web", "Facoltativo", "Scrivi un commento...", "Ancora nessun commento.", "Invia", "Mi piace", "Annulla mi piace", "Rispondi", "Annulla risposta", "Commenti", "Aggiorna", "Carica altri...", "Anteprima", "Emoji", "Carica immagine", "secondi fa", "minuti fa", "ore fa", "giorni fa", "adesso", "Caricamento in corso...", "Accedi", "Esci", "Admin", "In evidenza", "Parole", `Inserisci un commento tra $0 e $1 parole!
 Numero attuale di parole: $2`, "Anonimo", "Nano", "Hobbit", "Ent", "Mago", "Elfo", "Maiar", "GIF", "Cerca GIF", "Profilo", "Approvato", "In attesa", "Spam", "Rimuovi evidenza", "Pi\xF9 vecchi", "Pi\xF9 recenti", "Pi\xF9 popolari", "Cosa ne pensi?"]);
...
\u76EE\u524D\u5B57\u6578\uFF1A$2`, "\u533F\u540D", "\u6F5B\u6C34", "\u5192\u6CE1", "\u5410\u69FD", "\u6D3B\u8E8D", "\u591A\u8A71", "\u50B3\u8AAA", "\u8868\u60C5\u5305", "\u641C\u5C0B\u8868\u60C5\u5305", "\u500B\u4EBA\u8CC7\u6599", "\u901A\u904E", "\u5F85\u5BE9\u6838", "\u5783\u573E", "\u53D6\u6D88\u7F6E\u9802", "\u6700\u65E9", "\u6700\u65B0", "\u71B1\u9580", "\u4F60\u8A8D\u70BA\u9019\u7BC7\u6587\u7AE0\u600E\u9EBC\u6A23\uFF1F"]);
  ...
  var tt2 = (e2) => fe2[e2.toLowerCase()] || fe2[et.toLowerCase()];
  ...
  var va = ({ serverURL: e2, path: l3 = location.pathname, lang: t2 = typeof navigator > "u" ? "en-US" : navigator.language, locale: a, meta: n2 = ["nick", "mail", "link"], requiredMeta: o2 = [], dark: p3 = false, pageSize: s2 = 10, wordLimit: g, noCopyright: u2 = false, login: w3 = "enable", recaptchaV3Key: v3 = "", turnstileKey: S2 = "", commentSorting: j3 = "latest", emoji: L2 = De2, imageUploader: C3, highlighter: x2, texRenderer: R2, search: d3, reaction: K2, ...Y }) => ({ serverURL: ye2(e2), path: nt(l3), lang: at(t2), locale: { ...tt2(at(t2)), ...typeof a == "object" ? a : {} }, wordLimit: ma(g), meta: Be2(n2), requiredMeta: Be2(o2), dark: p3, pageSize: s2, commentSorting: j3, login: w3, noCopyright: u2, recaptchaV3Key: v3, turnstileKey: S2, ...Y, reaction: K2 === true ? Xt : K2 || null, imageUploader: ce2(C3, Yt), highlighter: ce2(x2, ia), texRenderer: ce2(R2, Jt), emoji: ce2(L2, De2), search: ce2(d3, Qt(t2)) });
  ...
  var ga = (e2, l3, t2) => {
    ...
    if (o2 === 0) {
      const p3 = n2 % 864e5, s2 = Math.floor(p3 / (3600 * 1e3));
      if (s2 === 0) {
        const g = p3 % 36e5, u2 = Math.floor(g / (60 * 1e3));
        if (u2 === 0) {
          const w3 = g % 6e4;
  ...
  var ka = (e2) => Promise.all(e2 ? e2.map((l3) => ie2(l3) ? ba(it(l3)) : Promise.resolve(l3)) : []).then((l3) => {
    const t2 = { tabs: [], map: {} };
    return l3.forEach((a) => {
      const { name: n2, folder: o2, icon: p3, prefix: s2 = "", type: g, items: u2 } = a;
      t2.tabs.push({ name: n2, icon: st(p3, o2, s2, g), items: u2.map((w3) => {
        const v3 = `${s2}${w3}`;
  ...
  var vt = ({ serverURL: e2, path: l3 = window.location.pathname, selector: t2 = ".waline-comment-count", lang: a = navigator.language }) => {
    ...
    return o2.length && f({ serverURL: ye2(e2), paths: Array.from(o2).map((p3) => nt(xe(p3) ?? l3)), lang: a, signal: n2.signal }).then((p3) => {
      o2.forEach((s2, g) => {
        s2.innerText = p3[g].toString();
      });
  ...
  var sl = defineComponent({ __name: "ArticleReaction", setup(e2) {
    const l3 = Za(), t2 = inject(we2), a = ref(-1), n2 = ref([]), o2 = computed2(() => t2.value.locale), p3 = computed2(() => {
      const { reaction: v3 } = t2.value;
      return v3 != null && v3.length ? v3 : null;
    }), s2 = computed2(() => {
      var v3;
      const { path: S2 } = t2.value;
      return ((v3 = p3.value) == null ? void 0 : v3.map((j3, L2) => ({ icon: j3, desc: o2.value[`reaction${L2}`], active: l3.value[S2] === L2 }))) ?? null;
    });
    let g;
    const u2 = async () => {
      ...
      g = L2.abort.bind(L2);
      const [C3] = await p({ serverURL: v3, lang: S2, paths: [j3], type: p3.value.map((x2, R2) => `reaction${R2}`), signal: L2.signal });
      n2.value = p3.value.map((x2, R2) => C3[`reaction${R2}`]);
    }, w3 = async (v3) => {
      if (a.value !== -1) return;
  ...
  var ml = defineComponent({ __name: "ImageWall", props: { items: { default: () => [] }, columnWidth: { default: 300 }, gap: { default: 0 } }, emits: ["insert"], setup(e2) {
    const l3 = e2;
    let t2 = null;
    const a = useTemplateRef("wall"), n2 = ref({}), o2 = ref([]), p3 = () => {
      ...
      if (o2.value.length === p3() && !v3) return;
      o2.value = s2(p3());
    ...
    return onMounted(() => {
      u2(true), t2 = new ResizeObserver(() => {
        u2();
      }), t2.observe(a.value), watch2(() => [l3.items], () => {
        n2.value = {}, u2(true);
      }), watch2(() => [l3.columnWidth, l3.gap], () => {
        u2();
      });
    }), onBeforeUnmount(() => {
  ...
  var ht = defineComponent({ __name: "CommentBox", props: { edit: {}, rootId: {}, replyId: {}, replyUser: {} }, emits: ["log", "cancelEdit", "cancelReply", "submit"], setup(e2, { emit: l3 }) {
    const t2 = e2, a = l3, n2 = inject(we2), o2 = Da(), p3 = Ba(), s2 = be2(), g = ref({}), u2 = useTemplateRef("textarea"), w3 = useTemplateRef("image-uploader"), v3 = useTemplateRef("emoji-button"), S2 = useTemplateRef("emoji-popup"), j3 = useTemplateRef("gif-button"), L2 = useTemplateRef("gif-popup"), C3 = useTemplateRef("gif-search"), x2 = ref({ tabs: [], map: {} }), R2 = ref(0), d3 = ref(false), K2 = ref(false), Y = ref(false), A = ref(""), V2 = ref(0), T2 = reactive({ loading: true, list: [] }), le2 = ref(0), ee = ref(false), me2 = ref(""), f2 = ref(false), z2 = ref(false), y3 = computed2(() => n2.value.locale), F2 = computed2(() => !!s2.value.token), q2 = computed2(() => n2.value.imageUploader !== null), O = (h2) => {
...
![${h2.name}](${k2})`);
      ...
      var h2;
      const { serverURL: i, lang: k2, login: _2, wordLimit: b2, requiredMeta: B2, recaptchaV3Key: N2, turnstileKey: J2 } = n2.value, H = { comment: me2.value, nick: p3.value.nick, mail: p3.value.mail, link: p3.value.link, url: n2.value.path, ua: await Ra() };
  ...
  var fn = defineComponent({ __name: "CommentCard", props: { comment: {}, edit: {}, rootId: {}, reply: {} }, emits: ["log", "submit", "delete", "like", "sticky", "edit", "reply", "status"], setup(e2, { emit: l3 }) {
    const t2 = e2, a = l3, n2 = ["approved", "waiting", "spam"], o2 = inject(we2), p3 = pt(), s2 = useNow(), g = be2(), u2 = computed2(() => o2.value.locale), w3 = computed2(() => {
      const { link: R2 } = t2.comment;
      return R2 ? rt(R2) ? R2 : `https://${R2}` : "";
    }), v3 = computed2(() => p3.value.includes(t2.comment.objectId)), S2 = computed2(() => ga(new Date(t2.comment.time), s2.value, u2.value)), j3 = computed2(() => g.value.type === "administrator"), L2 = computed2(() => t2.comment.user_id && g.value.objectId === t2.comment.user_id), C3 = computed2(() => {
      var R2;
  ...
  var jn = defineComponent({ __name: "WalineComment", props: { serverURL: {}, path: {}, meta: {}, requiredMeta: {}, wordLimit: {}, pageSize: {}, lang: {}, locale: {}, commentSorting: {}, dark: { type: [String, Boolean] }, login: {}, noCopyright: { type: Boolean }, recaptchaV3Key: {}, turnstileKey: {}, reaction: { type: [Array, Boolean] }, emoji: {}, search: {}, highlighter: { type: Function }, imageUploader: { type: Function }, texRenderer: { type: Function } }, setup(e2) {
    const l3 = e2, t2 = be2(), a = pt(), n2 = ref("loading"), o2 = ref(0), p3 = ref(1), s2 = ref(0), g = computed2(() => va(l3)), u2 = ref(g.value.commentSorting), w3 = ref([]), v3 = ref(null), S2 = ref(null), j3 = computed2(() => da(g.value.dark)), L2 = computed2(() => g.value.locale);
    ...
    const x2 = (f2) => {
      ...
      n2.value = "loading", C3 == null || C3(), $({ serverURL: z2, lang: g.value.lang, path: y3, pageSize: F2, sortBy: lt[u2.value], page: f2, signal: q2.signal, token: t2.value.token }).then((O) => {
        n2.value = "success", o2.value = O.count, w3.value.push(...O.data), p3.value = f2, s2.value = O.totalPages;
      }).catch((O) => {
  ...
  var yt = ({ serverURL: e2, path: l3 = window.location.pathname, selector: t2 = ".waline-pageview-count", update: a = true, lang: n2 = navigator.language }) => {
    const o2 = new AbortController(), p3 = Array.from(document.querySelectorAll(t2)), s2 = (u2) => {
      const w3 = xe(u2);
      return w3 !== null && l3 !== w3;
    }, g = (u2) => j({ serverURL: ye2(e2), paths: u2.map((w3) => xe(w3) ?? l3), lang: n2, signal: o2.signal }).then((w3) => wt(w3, u2)).catch(ct);
    if (a) {
      const u2 = p3.filter((v3) => !s2(v3)), w3 = p3.filter(s2);
  ...
  var zn = ({ el: e2 = "#waline", path: l3 = window.location.pathname, comment: t2 = false, pageview: a = false, ...n2 }) => {
    ...
    const p3 = reactive({ ...n2 }), s2 = reactive({ comment: t2, pageview: a, path: l3 }), g = () => {
      s2.comment && vt({ serverURL: p3.serverURL, path: s2.path, ...ie2(s2.comment) ? { selector: s2.comment } : {} });
    }, u2 = () => {
      s2.pageview && yt({ serverURL: p3.serverURL, path: s2.path, ...ie2(s2.pageview) ? { selector: s2.pageview } : {} });
    };
    let w3 = null;
    o2 && (w3 = createApp(() => h(jn, { path: s2.path, ...p3 })), w3.mount(o2));
